  ...
  describe 'POST /api/v1/login' do
    context 'with valid credentials' do
      it 'logs in the user and returns a token' do
        allow(UserService).to receive(:login).and_return({ success: true, user: { id: user.id, email: user.email }, token: 'fake-token' })
    ...
    context 'with invalid credentials' do
      it 'returns unauthorized' do
        allow(UserService).to receive(:login).and_return({ success: false, error: 'Invalid credentials' })
        post '/api/v1/login', params: { email: 'test@example.com', password: 'wrongpass' }
