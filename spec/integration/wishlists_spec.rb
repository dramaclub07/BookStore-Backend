...
<<<<<<< HEAD
  before do
    # Mock the WishlistService responses
    allow_any_instance_of(WishlistService).to receive(:toggle_wishlist).and_return(
  ...
  describe 'POST #toggle' do
    it 'toggles wishlist successfully' do
      post '/api/v1/wishlists', params: { book_id: book.id }, headers: headers
      expect(response).to have_http_status(:ok)
=======
  ...
  describe 'GET #index' do
    ...
    context 'when user is not found' do
      let(:non_existent_user_id) { (User.maximum(:id) || 0) + 1 }
      let(:invalid_token) { JwtService.encode_access_token(user_id: non_existent_user_id, exp: 1.hour.from_now.to_i) }
...
>>>>>>> 3c7d65f07fdaeb539ce1ffa598514cb8c3b3f26e
    end
  end

<<<<<<< HEAD
  describe 'GET #index' do
    it 'returns wishlist successfully' do
      get '/api/v1/wishlists', headers: headers
      expect(response).to have_http_status(:ok)
=======
    context 'when user is not authenticated' do
